#!/usr/bin/env python3
"""
OpenClaw Expenses 简化认证系统 - 使用SHA256 + JWT
避免bcrypt复杂问题，使用更简单的密码验证
"""
...
# 创建FastAPI应用实例
app = FastAPI(
    title="OpenClaw Expenses API with Simple Auth",
    version="2.0.0",
    description="个人支出数据管理RESTful API - 简化认证系统",
    docs_url="/api/docs",
    redoc_url="/api/redoc"
)
...
def init_auth_database():
    ...
    try:
        ...
        VALUES (
            'admin@openclaw.com',
            'admin',
            'salt$' || SHA2(CONCAT('admin123', 'salt'), 256),
            '系统管理员',
            TRUE,
            TRUE,
            TRUE
        ...
        VALUES (
            'test@openclaw.com',
            'testuser',
            'salt$' || SHA2(CONCAT('test123', 'salt'), 256),
            '测试用户',
            TRUE,
            FALSE,
            TRUE
